...
#[tauri::command]
pub async fn search_clipboard_history(query: String, limit: Option<i32>, offset: Option<i32>, content_filter: Option<String>) -> Vec<ClipboardItem> {
...
fn search_clipboard_history_sync(query: String, limit: Option<i32>, offset: Option<i32>, content_filter: Option<String>) -> Vec<ClipboardItem> {
    let conn = database::init_db();
    ...
    let category_condition = if is_all_filter {
        ...
        let placeholders: Vec<String> = filters.iter()
            .map(|f| format!("'{}'", f))
            .collect();
        // NULL kategorileri text olarak kabul et
        if filters.contains(&"text") {
            format!(" AND (COALESCE(category, 'text') IN ({}))", placeholders.join(","))
        } else {
    ...
    let rows: Vec<_> = match stmt.query_map([limit_value, offset_value], |row| {
        Ok((
            ...
            row.get::<_, String>(1)?,
            row.get::<_, String>(2)?,
            row.get::<_, Option<String>>(3)?.unwrap_or_else(|| "text".to_string()),
    ...
    rows.into_par_iter()
        .filter_map(|(id, content, content_type, category, image_data, created_at, pinned, is_encrypted)| {
...
fn search_without_category(conn: &rusqlite::Connection, query: &str, limit: i32, offset: i32, filters: &[&str]) -> Vec<ClipboardItem> {
    ...
    rows.into_iter()
        .filter_map(|(id, content, content_type, image_data, created_at, pinned, is_encrypted)| {
...
fn detect_category_from_content(content: &str, content_type: &str) -> String {
    ...
    if content_lower.starts_with("http://") || content_lower.starts_with("https://") || content_lower.starts_with("www.") {
    ...
    if content.contains("function") || content.contains("const ") || content.contains("let ") ||
       content.contains("def ") || content.contains("class ") || content.contains("fn ") ||
       content.contains("pub ") || content.contains("->") || content.contains("=>") ||
...
pub async fn get_clipboard_history(limit: Option<i32>, offset: Option<i32>) -> Vec<ClipboardItem> {
    async_runtime::spawn_blocking(move || {
        get_clipboard_history_sync(limit, offset)
    }).await.unwrap_or_else(|_| Vec::new())
}
...
fn get_clipboard_history_sync(limit: Option<i32>, offset: Option<i32>) -> Vec<ClipboardItem> {
    ...
    let sql_new = "SELECT id, content, content_type, category, image_data, created_at, pinned, is_encrypted 
                   FROM clipboard_history ORDER BY pinned DESC, id DESC LIMIT ?1 OFFSET ?2";
    ...
    if let Ok(mut stmt) = conn.prepare(sql_new) {
        ...
        return rows.into_iter().map(|(id, content, content_type, category, image_data, created_at, pinned, is_encrypted)| {
            let decrypted_content = if is_encrypted {
                security::decrypt(&content).unwrap_or(content)
            } else {
                content
    ...
    .map(|(id, content, content_type, image_data, created_at, pinned, is_encrypted)| {
        let decrypted_content = if is_encrypted {
            security::decrypt(&content).unwrap_or(content)
        } else {
            content
...
pub fn export_clipboard_history() -> Result<String, String> {
    ...
    let mut stmt = match conn.prepare(sql) {
        ...
        Err(_) => {
            ...
            let mut stmt = conn.prepare(sql_old).map_err(|e| format!("Prepare failed: {}", e))?;
